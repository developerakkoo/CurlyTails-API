    ...
    if (!savedCart.cartItems.length == 0) {
        ...
        for(let itemNo = 0; itemNo < savedCart.cartItems.length; itemNo++){
            ...
            if(savedCart.cartItems[itemNo].productId == req.body.productId){
                ...
                savedCart.TotalItems = savedCart.TotalItems+=req.body.quantity != undefined
                ? req.body.quantity
                : savedCart.TotalItems
            
                savedCart.SubTotal = savedCart.SubTotal+=(req.body.price*req.body.quantity) != undefined
...
exports.updateCartProductQuantity = async(req,res) =>{
    ...
    return res.status(404).json({message:'Product Not Found With This Product Id',statusCode:404,productId:req.body.productId});
...
exports.deleteProductFromCart = async(req,res) =>{
    ...
    savedCart.TotalItems = total != undefined
    ? total
    : savedCart.TotalItems

    savedCart.SubTotal = savedCart.SubTotal-=(result[0].price*result[0].quantity) != undefined
...
exports.getAllCarts = async(req,res) => {
    try {
        ...
        if (savedCart.length ==0) {
            return res.status(404).json({message:'Carts Not Found',statusCode:404});
        }
        res.status(200).json({message:'All Carts Fetched Successfully',statusCode:200,length:savedCart.length,data:savedCart});
    } catch (error) {
        console.log();
        res.status(500).json({message:error.message,statusCode:500,status:'ERROR'});
    }
...
exports.getCartById = async(req,res) => {
    try {
        ...
        if (!savedCart) {
            return res.status(404).json({message:`Cart Not Found With This cartId:${req.params.cartId}`,statusCode:404});
        }
        res.status(200).json({message:'Cart Fetched Successfully',statusCode:200,data:savedCart});
    } catch (error) {
        console.log();
        res.status(500).json({message:error.message,statusCode:500,status:'ERROR'});
    }
