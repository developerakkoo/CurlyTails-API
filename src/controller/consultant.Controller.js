...
exports.getAllConsultant = async (req, res) => {
  try {
    ...
    if (savedConsultant.length == 0) {
      return res
        .status(404)
        .json({ message: "Consultant Not Found", statusCode: 404 });
    }
    return res.status(200).json({
      message: "Consultant Fetched Successfully",
      statusCode: 200,
      length: savedConsultant.length,
...
exports.getConsultantById = async (req, res) => {
  try {
    const savedConsultant = await Consultant.findOne({
...
exports.updatedConsultant = async (req, res) => {
  try {
    ...
    savedConsultant.Name =
      req.body.name != undefined ? req.body.name : savedConsultant.Name;
    ...
    savedConsultant.isTopConsultant =
      req.body.isTopConsultant != undefined
...
exports.updateConsultantImage = async (req, res, next) => {
  try {
    ...
    savedConsultant.imageUrl =
      imageUrl != undefined ? imageUrl : savedConsultant.imageUrl;
    const updatedConsultant = await savedConsultant.save();
    res.status(201).json({
...
exports.getTopConsultant = async (req, res) => {
  try {
    ...
    if (savedConsultant.length == 0) {
      return res
        .status(404)
        .json({ message: "Top Consultant Not Found", statusCode: 404 });
    }
    return res.status(200).json({
