...
exports.addBanner = async (req, res) => {
  try {
    if (!req.file) {
      return res
        .status(400)
        .json({ message: "Please Provide Image File", statusCode: 400 });
    }
    let bannerObj = {
      imageUrl: `${req.protocol}://${req.hostname}/${req.file.path.replace(
        /\\/g,
        "/"
      )}`,
    };
...
exports.getBanners = async (req, res, next) => {
  try {
    ...
    if (banner.length == 0) {
      return res
        .status(404)
        .json({ message: "Banner Not Found", status: false, statusCode: 404 });
    }
    res.status(200).json({
...
exports.updateBanner = async (req, res, next) => {
  try {
    ...
    if (!banner) {
      return res.status(404).json({ msg: "Banner Not Found", statusCode: 404 });
    }
