...
exports.updateImage = async (req, res) => {
  try {
    ...
    const savedPharmacy = await Pharmacy.findOne({
      _id: req.params.pharmacyId,
    });
    if (!savedPharmacy) {
      return res
        .status(404)
        .json({ message: "Pharmacy Not Found", statusCode: 404 });
    }
    ...
    savedPharmacy.imageUrl =
      imageUrl != undefined ? imageUrl : savedPharmacy.imageUrl;
...
exports.updatedPharmacy = async (req, res) => {
  try {
    ...
    savedPharmacy.medicineName =
      req.body.medicineName != undefined
    ...
    savedPharmacy.price =
      req.body.price != undefined ? req.body.price : savedPharmacy.price;

    savedPharmacy.Stock =
      req.body.Stock != undefined ? req.body.Stock : savedPharmacy.Stock;

    savedPharmacy.inStock =
      req.body.inStock != undefined ? req.body.inStock : savedPharmacy.inStock;
