...
exports.getAllRefundRequest = async (req, res) => {
  try {
    ...
    if (savedRefund.length == 0) {
      return res
        .status(404)
        .json({ message: `Refund Requests Not Found`, statusCode: 404 });
    }
    res.status(200).json({
      message: "Refund Request Fetched Successfully",
      statusCode: 200,
      length: savedRefund.length,
      data: savedRefund,
    });
  } catch (error) {
    res
...
exports.getRefundRequestById = async (req, res) => {
  try {
    ...
    if (!savedRefund) {
      return res
        .status(404)
        .json({ message: `Refund Request Not Found`, statusCode: 404 });
    }
    res.status(200).json({
      message: "Refund Request Fetched Successfully",
      statusCode: 200,
...
exports.updateRefundRequestStatus = async (req, res) => {
  try {
    ...
    if (req.body.status == "REFUND_INITIATED") {
      savedRefund.refundStatus =
        status.Accept != undefined
          ? status.Accept
